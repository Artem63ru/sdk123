...
class Ref_obj extends Model
{
    ...
    public $primaryKey = 'buildingGUID';

    public function elem_to_jas()
    {
        return $this->hasMany('App\Jas', 'from_elem_opo', 'buildingGUID');
//        return $this->hasMany('App\Jas', 'idOPO', 'from_opo');
    ...
    {
        return $this->hasMany('App\Models\Calc_elem', 'from_elem', 'buildingGUID')->orderByDesc('id');
    }
    //************************** Последнее значение расчетных параметров для елемента ОПО **********************************************
    public function elem_to_calc()
    {
        return $this->hasMany('App\Models\Calc_elem', 'from_elem', 'buildingGUID')->orderByDesc('id')->take(1);

    }
    //************************** Последнее значение расчетных параметров 40 шт для елемента ОПО **********************************************
    public function elem_to_calc_40()
    {
        return $this->hasMany('App\Models\Calc_elem', 'from_elem', 'buildingGUID')->orderByDesc('id')->select('ip_elem','op_m','op_r', 'op_el','date')->take(40);
    ...
    {
        return $this->hasMany('App\Models\Dynamic\Calc_tb', 'from_obj', 'buildingGUID')->orderByDesc('id')->take(100);

    }
    public function elem_to_oto()
    {
        return $this->hasMany('App\Models\ref_oto', 'typeObj', 'buildingGUID');

    }
    public function elem_to_tu()
    {
        return $this->hasMany('App\Models\Tu', 'from_el_opo', 'buildingGUID');
    ...
    {
        return $this->hasMany('App\Models\APK_SDK', 'idObj', 'buildingGUID');
    }
    ...
    protected $fillable = [
        'buildingGUID ', 'nameObj', 'uppg', 'InUse', 'QP1_TYPE', 'idOPO', 'buildingName', 'typeObj', 'status', 'type_project',
        'buildingState', 'buildingStatus', 'buildingDateOfConstraction', 'buildingLaunchDate', 'buildingDateOfCR', 'buildingDateOfReconstruction', 'buildingIn', 'buildingFirstLifeDate', 'pbRegNum', 'pbState',
        'pbIssueDate', 'pbValidUntil', 'nextPbDate', 'buildingLifeProlongationFact', 'buildingLifeProlongationExNum', 'buildingLifeProlongationExDate',
        'buildingLifeProlongationDate', 'buildingWear',
    ];
